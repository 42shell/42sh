#!/usr/bin/env bash
# progress_bar - A pacman like progress bar in bash
...
__progress_bar_error() {
 echo "A valid ${*} must be supplied to initialise 'progress_bar'."
}

__progress_bar_theme() {
...
progress_bar() {
 [[ -z "${1}" ]] && __die "$(__progress_bar_error bar title)"
 [[ -z "${2}" ]] && __die "$(__progress_bar_error curent position)"
